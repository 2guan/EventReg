...
1.  **克隆项目**
    ```bash
    git clone https://github.com/2guan/EventReg.git
    cd EventReg
    ```
...
```text
EventReg/
├── public/              # 静态资源（图片、头像、Banner）
├── server/              # 后端源码
│   ├── routes/          # API 路由（auth、matches、users 等）
│   ├── db.js            # 数据库连接与初始化
│   ├── index.js         # 后端入口
...
```text
EventReg/
├── public/              # Static assets (images, faces, banners)
├── server/              # Backend source code
│   ├── routes/          # API Routes (auth, matches, users, etc.)
...
│       ├── MyScores.tsx # User Profile
│       └── ...
├── eventregmini/        # WeChat Mini Program source
